...
case class Seqstats(positions: ArrayBuffer[Position] = ArrayBuffer(),
  ...
  def add(record: FastqRecord): Seqstats = {
    ...
    if (positions.size < length) positions.append(Array.fill(length - positions.size)(Position()):_*)
    record.getBaseQualities.zipWithIndex.foreach(x => positions(x._2).qual.add(x._1))
    record.getReadBases.zipWithIndex.foreach(x => positions(x._2).nuc.add(x._1))
    this
  }

  def +=(other: Seqstats): Seqstats = {
    this.readLength += other.readLength
    if (this.positions.size > other.positions.size) other.positions.append(Array.fill(this.positions.size - other.positions.size)(Position()):_*)
    if (this.positions.size < other.positions.size) this.positions.append(Array.fill(other.positions.size - this.positions.size)(Position()):_*)
